...
3. **`any(iterable)`**：如果可迭代对象 `iterable`
4. 中有任何元素为真（truthy），则返回 `True`。
5. **`ascii(obj)`**：返回对象 `obj` 的可打印字符表示形式。
6. **`bin(x)`**：将整数 `x` 转换为二进制字符串。
7. **`bool(x)`**：将 `x`转换为布尔值，非零数值、非空序列等为 `True`，否则为 `False`。
8. **`bytearray(iterable)`**：返回一个可变的字节数组，由 `iterable` 中的元素构成。
9. **`bytes(iterable)`**：返回一个不可变的字节序列，由 `iterable` 中的元素构成。
10. **`callable(obj)`**：如果 `obj` 是可调用对象（如函数、方法等），则返回 `True`。
11. **`chr(i)`**：返回整数 `i` 对应的 Unicode 字符。
12. **`classmethod(func)`**：将函数 `func` 转换为类方法。
13. **`compile(source, filename, mode)`**：将源代码 `source` 编译成代码对象。
14. **`complex(real, imag)`**：创建一个复数，其实部为 `real`，虚部为 `imag`。
15. **`delattr(obj, name)`**：删除对象 `obj` 的属性 `name`。
16. **`dict(**kwargs)`\*\*：创建并返回一个字典。
17. **`dir()`**：不带参数时，返回当前局部符号表的列表。带参数时，返回对象的属性列表。
18. **`divmod(a, b)`**：返回 `a` 除以 `b` 的商和余数。
19. **`enumerate(iterable, start=0)`**：返回一个枚举对象，它产生 `(index, value)` 对。
20. **`eval(expression, globals=None, locals=None)`**：计算字符串 `expression` 中的表达式，并返回结果。
21. **`exec(object, globals=None, locals=None)`**：执行动态生成的 Python 代码。
22. **`filter(function, iterable)`**：构造一个迭代器，返回使函数 `function` 返回值为真的那些元素。
23. **`float(x)`**：将 `x` 转换为浮点数。
24. **`format(value, format_spec)`**：将 `value` 格式化为字符串。
25. **`frozenset(iterable)`**：返回一个不可变的集合，由 `iterable`中的元素构成。
26. **`getattr(object, name[, default])`**：返回对象 `object`的属性 `name`，如果没有该属性则返回 `default`。
27. **`globals()`**：返回当前全局符号表的字典。
28. **`hasattr(object, name)`**：如果对象 `object` 有属性 `name`，则返回 `True`。
29. **`hash(object)`**：返回对象 `object` 的哈希值。
30. **`help(object)`**：显示对象 `object` 的帮助信息。
31. **`hex(x)`**：将整数 `x` 转换为十六进制字符串。
32. **`id(object)`**：返回对象 `object` 的唯一标识符。
33. **`input(prompt='')`**：显示提示信息 `prompt` 并返回用户输入的字符串。
34. **`int(x[, base])`**：将 `x` 转换为整数，可选参数 `base` 指定进制。
35. **`isinstance(object, classinfo)`**：如果 `object` 是 `classinfo` 的实例或其子类的实例，则返回 `True`。
36. **`issubclass(C, B)`**：如果类 `C` 是类 `B` 的子类，则返回 `True`。
37. **`iter(iterable, sentinel)`**：返回迭代器对象，可选参数 `sentinel` 指定迭代结束条件。
38. **`len(object)`**：返回对象 `object` 的长度。
39. **`list(iterable)`**：返回列表，由 `iterable` 中的元素构成。
40. **`locals()`**：返回当前局部符号表的字典。
41. **`map(function, iterable, ...)`**：将函数 `function` 应用于 `iterable` 中的每个元素，并返回结果的迭代器。
42. **`max(iterable, *args, key=None, default=None)`**：返回 `iterable` 中的最大值。
43. **`memoryview(object)`**：返回对象的内存视图。
44. **`min(iterable, *args, key=None, default=None)`**：返回 `iterable` 中的最小值。
45. **`next(iterator[, default])`**：返回迭代器的下一个元素，如果没有更多元素则返回 `default`。
46. **`object()`**：返回一个新的、未命名的对象。
47. **`oct(x)`**：将整数 `x` 转换为八进制字符串。
48. **`open(file, mode='r', ...)`**：打开文件并返回文件对象。
49. **`ord(c)`**：返回字符 `c` 的 Unicode 码点。
50. **`pow(base, exp[, mod])`**：返回 `base` 的 `exp` 次幂，可选参数 `mod` 指定取模。
51. **`print(*args, sep=' ', end='\n', file=sys.stdout, flush=False)`**：打印 `args` 中的值，用 `sep` 分隔，以 `end` 结尾。
52. **`property(fget=None, fset=None, fdel=None, doc=None)`**：返回一个属性对象。
53. **`range(start, stop[, step])`**：返回一个整数序列。
54. **`repr(object)`**：返回对象的“官方”字符串表示形式。
55. **`reversed(seq)`**：返回一个反向迭代器。
56. **`round(number[, ndigits])`**：返回浮点数 `number` 四舍五入到小数点后 `ndigits` 位的值。
57. **`set(iterable)`**：返回一个集合，由 `iterable` 中的元素构成。
58. **`setattr(object, name, value)`**：设置对象 `object` 的属性 `name` 为 `value`。
59. **`slice(stop)`**：返回一个切片对象。
60. **`sorted(iterable, *, key=None, reverse=False)`**：返回 `iterable` 的排序列表。
61. **`staticmethod(function)`**：将函数 `function` 转换为静态方法。
62. **`str(object='）`**：将 `object` 转换为字符串。
63. **`sum(iterable, start=0)`**：返回 `iterable` 中元素的总和。
64. **`super()`**：返回一个代理对象，用于调用父类的方法。
65. **`tuple(iterable)`**：返回一个元组，由 `iterable` 中的元素构成。
66. **`type(name, bases, dict)`**：返回一个新的类型对象。
67. **`vars(object)`**：返回对象的 `__dict__` 属性。
68. **`zip(iter1, ..., iterN)`**：将多个迭代器中对应的元素打包成一个个元组，然后返回由这些元组组成的迭代器。
69. **`__import__(name, globals=None, locals=None, fromlist=(), level=0)`**：动态导入模块。
