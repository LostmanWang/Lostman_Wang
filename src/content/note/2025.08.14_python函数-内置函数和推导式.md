---
title: Python函数-匿名函数、生成器、内置函数和推导式
...
1. **列表推导式**：这个表达式创建了一个列表，其中包含三个 `lambda` 表达式。
2. **`lambda` 表达式**：每个 `lambda` 表达式都引用了变量 `i`，这是在列表推导式外部定义的。
3. **闭包形成**：由于每个 `lambda` 表达式都引用了外部变量 `i`，Python 需要记住每个 `lambda` 表达式被创建时 `i` 的值。这是通过创建一个 `cell` 对象来实现的，该对象存储了 `i` 的值，并在 `lambda` 表达式被调用时提供这个值。
