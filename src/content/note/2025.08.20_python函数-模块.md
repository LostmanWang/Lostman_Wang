...
| 方式               | 命令/工具                                                        | 适用场景                  | 特点                         |
| ---------------- | ------------------------------------------------------------ | --------------------- | -------------------------- |
| pip（官方推荐）        | `python -m pip install 包名`                                   | 绝大多数情况                | 直接连接 PyPI，支持版本控制、依赖解析      |
| conda            | `conda install 包名`                                           | 使用 Anaconda/Miniconda | 同时解决二进制依赖（C/Fortran 库），跨平台 |
| 清华/阿里/豆瓣镜像       | `pip install -i https://pypi.tuna.tsinghua.edu.cn/simple 包名` | 国内网络慢                 | 速度快，需加 `--trusted-host`    |
| requirements.txt | `pip install -r requirements.txt`                            | 项目部署/团队协作             | 锁定版本，一键复现环境                |
| 源码安装             | `python setup.py install` 或 `pip install .`                  | 源码修改/未发布版本            | 需编译环境，耗时                   |
| 开发模式             | `pip install -e .`                                           | 模块开发调试                | 软链接到源码目录，实时生效              |
| 私有仓库             | `pip install --extra-index-url https://myrepo/simple 包名`     | 公司内网/私有包              | 安全可控                       |
| 虚拟环境             | 先 `python -m venv venv` → 激活 → `pip install ...`             | 避免全局污染                | 每个项目独立依赖树                  |
